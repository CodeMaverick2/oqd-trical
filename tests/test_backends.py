...
class TestBackendParity:
    ...
    def test_task_args_atomic_emulator_solver_options(self):
        opts = DynamiqsSolverOptions(rtol=1e-4)
        args = TaskArgsAtomicEmulator(
            fock_trunc=FOCK_CUTOFF, dt=TIMESTEP, dynamiqs_solver_options=opts
        )
        backend = DynamiqsBackend(
            approx_pass=_approx_pass(_microwave_rabi_circuit()[1]),
            solver_options=args.dynamiqs_solver_options,
        )
        assert backend.solver_options["method"].rtol == 1e-4

    def test_jax_hamiltonian_evaluation(self):
        import jax
        from jax import numpy as jnp

        circuit, frame_specs = _microwave_rabi_circuit()
        exp, _ = DynamiqsBackend(approx_pass=_approx_pass(frame_specs)).compile(
            circuit, FOCK_CUTOFF
        )
        H = exp.sequence[0].hamiltonian

        @jax.jit
        def norm_at_zero():
            return jnp.max(jnp.abs(H(0.0).to_jax()))

        assert float(norm_at_zero()) > 0.0
